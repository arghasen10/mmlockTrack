...
def get_aug_pipe(frame_stack=10, aug=True):
...
class Dataset:
    def __init__(self, loc="merged_df.pkl", frame_stack=10, augment=True):
        ...
        # class members
        if augment:
            self.class_count = df.groupby('Activity').size().to_numpy().max()
        else:
            self.class_count = df.groupby('Activity').size().to_numpy().min()

        self.do_num_aug = np.ceil(self.class_count / df['Activity'].value_counts()).to_dict()
        self.pipe = get_aug_pipe(frame_stack=frame_stack, aug=augment)
...
def get_dataset():
    data = Dataset(loc='merged_df.pkl',
                   frame_stack=10,
                   augment=False)
...
model = get_model()
model.load_weights('saved_weights_macro_no_aug.h5')
# model.compile(loss="categorical_crossentropy", optimizer='adam',metrics="accuracy")

# folder=datetime.datetime.now().strftime("%Y_%m_%d-%I:%M:%S_%p")
# best_save=tf.keras.callbacks.ModelCheckpoint(filepath='saved_weights_macro_no_aug.h5',save_weights_only=True,
