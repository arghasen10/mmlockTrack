...
yticks = np.linspace(-8, 8, 8, dtype=np.int)
xticks = np.linspace(256, 0, 10, dtype=np.int)
...
yticks = np.linspace(16, 0, 8, dtype=np.int)
xticks = np.linspace(256, 0, 10, dtype=np.int)
g = sns.heatmap(doppz[0], ax=axs[0], cbar_ax=axs[1], vmax=1, vmin=0)
g.set_xlabel('Range bins')
g.set_ylabel('Doppler bins')
...
xticks = np.linspace(64, 0, 10, dtype=np.int)
